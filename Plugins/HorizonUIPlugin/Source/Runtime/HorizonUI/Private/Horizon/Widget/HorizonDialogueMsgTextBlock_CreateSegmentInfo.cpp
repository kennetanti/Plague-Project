...
bool ApplySegmentStyle(FHorizonDialogueSegmentInfo& segParam, const FString& segName, const TArray<FHorizonDialogueSegmentInfoStyle>& segmentStyleList)
{
...
FHorizonDialogueSegmentInfo UHorizonDialogueMsgTextBlock::CreateSegmentInfo(const FHorizonDialogueSegmentInfo& parentSegParam, const FXmlNode* pXmlNode) {
	...
	for (const auto& attrIt : attributeList) {
		...
		if (tag.Equals(TEXT("style"), ESearchCase::IgnoreCase)) {
			bool bStyleFound = ApplySegmentStyle(segParam, value, SegmentStyleList);


			if (!bStyleFound)
			{
				for (int i = 0; i < StyleInfoList.Num(); ++i)
				{
					auto& styleInfo = StyleInfoList[i];
					//if (styleInfo)
					{
						bStyleFound = ApplySegmentStyle(segParam, value, styleInfo->SegmentStyleList);
						if (bStyleFound)
						{
