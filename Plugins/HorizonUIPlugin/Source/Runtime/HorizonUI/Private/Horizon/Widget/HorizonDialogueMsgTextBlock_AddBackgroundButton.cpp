...
void UHorizonDialogueMsgTextBlock::SetSegmentColor(int segmentIndex, int lineIndex, const FSlateColor& InColor)
{
	auto& currentLine = DialogueLineInfoList[lineIndex];
	...
	if (pCanvasPanelSlot)
	{
		...
		if (pButton)
		{
			...
			while (UGameplayStatics::GrabOption(tempStr, Pair))
			{
				...
				hyperTextResult.UrlEncodeMap.Add(PairKey, PairValue);
			}
